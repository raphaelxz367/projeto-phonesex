...
if (instance_exists(obj_player)) {
    ...
    if (dist <= distancia_seguir) {
        ...
        if (dist < distancia_invocar && !cajado_criado) {
            instance_create_layer(x, y, "Instances", obj_cajado_do_mago_de_lua);
            cajado_criado = true;
        }
    }
}
...
if (instance_exists(obj_player)) {
    if (distance_to_object(obj_player) < distancia_ativar && !instance_exists(obj_cajado_do_mago_de_lua)) {
        instance_create_layer(x, y, "Instances_1", obj_cajado_do_mago_de_lua);
    }
}
#endregion
